<template>
  <div>
    ...
    >
      <div id="successbox">
        <div class="infotip">
          感谢您订购智能图片处理服务，您的订单已<span style="color: #67c23a"
            >支付成功！</span
          >
          您的智能图片处理服务专属链接为:
          {{ sealurl }}，请妥善保存，此链接已同步发送至下单时填写的电子邮箱中。
        </div>

        <div>
          本服务一经出售，<span style="color: red">不接受退款申请</span>
        </div>

        <div>
          <div class="infotip">
            本专属链接只能处理<span style="color: red">3</span>次图片，处理<span
              style="color: red"
              >3</span
            >次图片后，本链接<span style="color: red">自动失效</span>。
          </div>
          <div class="infotip">
            如果您需立即处理图片，可<span
              @click="openyw"
              style="color: #1a53ff; cursor: pointer"
              >点击此处</span
            >直接跳转到智能图片处理系统。
          </div>
        </div>
...
import { checkorderstatus, conf_list, getWebSocketPrefix } from "@/api/manage";
...
export default {
  ...
  methods: {
    openyw() {
      window.open(this.sealurl, "_blank");
    },
    onCopy() {
        ...
        checkorderstatus({ tradeno: apiRes.payOrderId }).then((res) => {
          ...
          switch (res.data.state) {
            case 2:
              ...
              this.sealurl =
                this.configdata.sealdomain + "?code=" + res.data.sealcodestr;
              break;
            case 0:
              break;
            default:
